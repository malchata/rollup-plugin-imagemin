...
function index (userOptions = {}) {
  ...
  const pluginOptions = _objectSpread2(_objectSpread2({}, defaultOptions), userOptions); // Merge user options with defaults for each plugin
  ...
  allPluginsFactoriesPairs.reduce((pluginOptionsAcc, [pluginName]) => {
    ...
    pluginOptionsAcc[pluginName] = _objectSpread2(_objectSpread2({}, defaultOptions[pluginName]), pluginUserOpts);
  ...
  const filter = rollupPluginutils.createFilter(pluginOptions.include, pluginOptions.exclude);
